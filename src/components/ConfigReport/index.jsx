...
import {Container, Box, Accordion, AccordionSummary, AccordionDetails, Button, IconButton} from '@mui/material';
...
function ConfigReport({state, dispatch, search, setSearch, searchResult}) {
    ...
    const SwitchModal = () => {
        dispatch({type: actionTypesModals.setModalConfig, payload: !StatesModals.setModalConfig})
      }
      ...
      >
        ...
        >
          ...
          <Box sx={{display:'flex', flexDirection:'row', width:'100%', gap:'10px', justifyContent:'space-between'}}>
            <Button variant="contained" onClick={GenerateDocument}>Vista Previa</Button>
            {validateContent && (<ButtonPDF/>)}
            {!validateContent && (<Button variant="contained" onClick={GenerateDocument}>Descargar</Button>)}
          </Box>

           {StatesModals.modalConfig && ( <UserConfig state={state} dispatch={dispatch}/>)}
